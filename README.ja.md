...
最初のExampleは、まず **作業前チェック** の半分だけを体験できるようにしています。別のHuman / AI / Harnessが実際の作業を行った後は、`nazeyatta verify request.yaml observation.yaml` で、期待した事実と観測された事実をexactに比較できます。

```text
nazeyatta check   = 作業前
nazeyatta verify  = 作業後
```

どちらのコマンドも、作業そのものを実行したり、実行権限や再試行権限を発行したりはしません。
...
> **「確認できていないのに進む」ことを防ぐ作業前チェックと、「言っていた結果と実際の結果が違う」を作業後にexact確認する仕組み**
...
- 外部の作業後に、期待した結果と実際の観測結果をexactに比較したい
...
- deploy・公開・外部writeなどの後に、実際の結果が期待どおりだったか確認したい時
...
PyPIからstable版 `1.1.0` を入れ、同梱Exampleを取り出します。
