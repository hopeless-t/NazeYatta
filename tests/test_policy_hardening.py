...
def test_exists_condition_requires_boolean():
    # A quoted YAML value such as `exists: "false"` must not become truthy via bool("false").
    policies = copy.deepcopy(POLICIES)
    policies["rules"][0]["when"]["all"][0] = {
        "field": "action.side_effect",
        "exists": "false",
    }
    with pytest.raises(PolicyError, match="'exists'.*boolean"):
